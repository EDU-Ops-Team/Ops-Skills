...
Rhode Island is often misclassified as notification-only. Baseline is **APPROVAL_REQUIRED**: RIDE must approve the school (eRIDE), with **annual** re-approval tied to the school-year cycle (**last working day of June** is the recurring submission deadline for the upcoming year — confirm the exact date each year on RIDE materials).

1. Search: `Rhode Island RIDE non-public school approval eRIDE site:ride.ri.gov`
2. Search: `Rhode Island private school substantially equivalent instructional time`
3. Confirm: approval **before** operating; substantially equivalent curriculum and instructional minutes vs. public schools; immunization/health record expectations; **staff criminal records review** (treat as `background_check_required` at the education-authority level when RIDE conditions approval on it)
4. Confirm teacher rules: general classroom teachers may be uncertificated; verify current RIDE guidelines for **any** degree or certification rules (e.g., special education) before setting `teacher_certification_required`
5. If opening timeline could miss the June cycle for the target school year, flag `"CALENDAR_RISK"` and populate `calendar_window` with the next June deadline and submission rules from official sources
...
| Background checks/fingerprinting | FL (owner fingerprinting required as part of DOE registration), RI (staff criminal records review tied to RIDE non-public approval), some others |
...
| RI | RIDE | Non-public school **approval** via eRIDE (initial + annual); listed in RIDE directory. Substantially equivalent instructional time/program; staff criminal records review. **Not** a notice-only state. | RIGL §16-19, §16-40, §16-60; verify `ride.ri.gov` Non-Public Schools |
...
| RI | 66 | APPROVAL_REQUIRED | CERTIFICATE_OR_APPROVAL_REQUIRED | Yes | 45 |
...
- **Rhode Island:** Treating RIDE non-public school approval as “notice only” — current practice is **approval** via eRIDE (initial + annual), with a **June** school-year cycle deadline; verify on `ride.ri.gov` before scoring.
...
9. `Providence, RI` → APPROVAL_REQUIRED, YELLOW (~66 baseline), `CERTIFICATE_OR_APPROVAL_REQUIRED`, gating=true, RIDE/eRIDE + June cycle noted, `background_check_required` true if staff records review confirmed
